...
#network --device=eth0 --bootproto=static --ip=192.168.0.1 --netmask=255.255.255.0 --gateway=192.168.0.254 --nameserver=8.8.8.8 --noipv6 --hostname=ubuntu-14-04
...
AUTOMATER_REPO=https://raw.githubusercontent.com/Mikroways/xenserver-automater
wget -O /usr/sbin/xe-set-hostname $AUTOMATER_REPO/master/usr/sbin/xe-set-hostname
wget -O /usr/sbin/xe-set-network $AUTOMATER_REPO/master/usr/sbin/xe-set-network
wget -O /usr/sbin/generate-sshd-keys $AUTOMATER_REPO/master/usr/sbin/generate-sshd-keys
wget -O /etc/init/xe-automator.conf $AUTOMATER_REPO/master/etc/init/xe-automator.conf
